...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if(gimbal_behaviour == GIMBAL_MINIPC_AUTOAIM_LOCK)
		{		
				gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK;
		}
}
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		{
				gimbal_miniPC_autoaim_lock_absolute_angle_control(add_yaw, add_pitch, gimbal_control_set);
		}
...
static void gimbal_miniPC_autoaim_lock_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
      ...
      return;
		...
		/* 重置PID 修改状态机切换 和 修改状态机的权限 */
		
		//不使用增量信号控制, MiniPC传过来的是阶跃信号
		*yaw = 0.0f;
		*pitch  = 0.0f;
		
/* 这部分没有用
//			*yaw = yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN + yawMove;
//			*pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN + pitchMove;
//			*yaw = miniPC_info.yawMove;
//			*pitch = miniPC_info.pitchMove;			
//			if( fabs((gimbal_control_set->gimbal_yaw_motor.miniPC_absolute_angle_set) - (gimbal_control_set->gimbal_yaw_motor.absolute_angle)) < 0.05)
//			{
//				gimbal_control_set->gimbal_yaw_motor.miniPC_absolute_angle_set = gimbal_control_set->gimbal_yaw_motor.absolute_angle;
//			}
//			
//			if( fabs((gimbal_control_set->gimbal_pitch_motor.miniPC_absolute_angle_set) - (gimbal_control_set->gimbal_pitch_motor.absolute_angle)) < 0.05)
//			{
//				gimbal_control_set->gimbal_pitch_motor.miniPC_absolute_angle_set = gimbal_control_set->gimbal_pitch_motor.absolute_angle;
//			}
			*/
...
//			{
//				temp_absolute_yaw = temp_last_absolute_yaw;
//			}
//			
//			if( fabs(temp_absolute_pitch - temp_last_absolute_pitch) < 0.05)
//			{
