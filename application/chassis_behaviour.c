...
//temp variable for chassis spin
...
/**
  ...
  * @retval         none
  */
//根据 这个底盘行为模式变量 判断 识别 分配 底盘chassis_move底盘模式状态机
//可能多个 底盘行为模式变 对应一个底盘状态机
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    {
        chassis_behaviour_mode = CHASSIS_ZERO_FORCE;//SZL 2-5-2022 之前是CHASSIS_NO_MOVE
    }
    else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
    {
				...
				else if(chassis_move_mode->chassis_RC->key.v & KEY_PRESSED_OFFSET_G)
				{
					  swing_flag = 0;
				}
    ...
    {
				//SZL 2-5-2022 修改 原来是CHASSIS_NO_MOVE
        chassis_behaviour_mode = CHASSIS_ZERO_FORCE;
				//之后修改的思路，底盘在转动时突然到chassis_behaviour_mode=CHASSIS_ZERO_FORCE，先底盘更随着一起刹车
...
/**
  ...
  * @retval         none
  */
////根据 这个底盘行为模式变量 判断 识别 分配 底盘的 目标 Vx Vy Vw
...
//    //judge if swing
//    //判断是否要360
//    if (chassis_move_rc_to_vector->chassis_RC->key.v & KEY_PRESSED_OFFSET_F)
//    {
//            swing_flag = 1;
//    }
//    else if(chassis_move_rc_to_vector->chassis_RC->key.v & KEY_PRESSED_OFFSET_G)
//    {
//        swing_flag = 0;
//    }
...
//    if (swing_flag)
//    {
//			// swing_angle = swing_angle + 0.005f;
//			
//    }
//    else
//    {
...
/**
  ...
  * @retval         返回空
  */
//底盘行为模式变 为 chassis_behaviour_mode == CHASSIS_NO_FOLLOW_YAW时
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_rc_to_vector == NULL)
    {
