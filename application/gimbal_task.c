...
/**
  ...
  * @retval         none
  */
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add, uint8_t bypass);

static void gimbal_miniPC_autoaim_lock_absolute_angle_limit(gimbal_motor_t *gimbal_motor, uint8_t bypass);

static void gimbal_motor_miniPC_autoaim_lock_absolute_angle_control(gimbal_motor_t *gimbal_motor);	
/**
...
static void gimbal_init(gimbal_control_t *init)
{
		/*SZL 6-12-2022添加, 自瞄绝对角度 角度环 PID*/
		//初始化yaw电机 MiniPC 角度环 pid
		gimbal_PID_init(&init->gimbal_yaw_motor.miniPC_absolute_angle_pid, YAW_MINIPC_ABSOLUTE_ANGLE_PID_MAX_OUT, YAW_MINIPC_ABSOLUTE_ANGLE_PID_MAX_IOUT, YAW_MINIPC_ABSOLUTE_ANGLE_PID_KP, YAW_MINIPC_ABSOLUTE_ANGLE_PID_KI, YAW_MINIPC_ABSOLUTE_ANGLE_PID_KD);
		//初始化pitch电机 MiniPC 角度环 pid
		gimbal_PID_init(&init->gimbal_pitch_motor.miniPC_absolute_angle_pid, PITCH_MINIPC_ABSOLUTE_ANGLE_PID_MAX_OUT, PITCH_MINIPC_ABSOLUTE_ANGLE_PID_MAX_IOUT, PITCH_MINIPC_ABSOLUTE_ANGLE_PID_KP, PITCH_MINIPC_ABSOLUTE_ANGLE_PID_KI, PITCH_MINIPC_ABSOLUTE_ANGLE_PID_KD);
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
    ...
    //云台数据更新
    feedback_update->gimbal_pitch_motor.absolute_angle = *(feedback_update->gimbal_INT_angle_point + INS_gimbal_angle_PITCH_ADDRESS_OFFSET);
    ...
    feedback_update->gimbal_pitch_motor.motor_gyro = *(feedback_update->gimbal_INT_gyro_point + INS_gimbal_gyro_PITCH_ADDRESS_OFFSET);

    feedback_update->gimbal_yaw_motor.absolute_angle = *(feedback_update->gimbal_INT_angle_point + INS_gimbal_angle_YAW_ADDRESS_OFFSET);
    ...
    feedback_update->gimbal_yaw_motor.relative_angle = motor_ecd_to_angle_change(feedback_update->gimbal_yaw_motor.gimbal_motor_measure->ecd,
                                                                                        feedback_update->gimbal_yaw_motor.offset_ecd);
#endif
    feedback_update->gimbal_yaw_motor.motor_gyro = arm_cos_f32(feedback_update->gimbal_pitch_motor.relative_angle) * (*(feedback_update->gimbal_INT_gyro_point + INS_gimbal_gyro_YAW_ADDRESS_OFFSET))
                                                        - arm_sin_f32(feedback_update->gimbal_pitch_motor.relative_angle) * (*(feedback_update->gimbal_INT_gyro_point + INS_gimbal_gyro_ROLL_ADDRESS_OFFSET));
}
/*SZL: 上面这句话: feedback_update->gimbal_pitch_motor.relative_angle = 相对于中间点的弧度制弧度
先看到这儿
*/
...
{
		/*
	  电机 角度计算 底层编码器 角度方向判断 <-> 里程计 就要依赖于这个东西
	  */
    int32_t relative_ecd = ecd - offset_ecd;
    if (relative_ecd > HALF_ECD_RANGE)//HALF_ECD_RANGE = 4096
    {
        relative_ecd -= ECD_RANGE;//ECD_RANGE = 8191
    }
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
    if (gimbal_mode_change == NULL)
    {
		...
		/*
		这样处理数据, 从而保证切换的时候, 云台不会抽风
		gimbal_mode_change->gimbal_yaw_motor.absolute_angle_set = gimbal_mode_change->gimbal_yaw_motor.absolute_angle;
		从而确保了切换时云台不会大幅度移动
		*/
    //yaw电机状态机切换保存数据
    if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_RAW && gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
    {//SZL 切换至 GIMBAL_MOTOR_RAW的时候
    ...
    {//切换至 GIMBAL_MOTOR_ENCONDE 的时候
        gimbal_mode_change->gimbal_yaw_motor.relative_angle_set = gimbal_mode_change->gimbal_yaw_motor.relative_angle;
    }
		else if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK && gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK)
		{//切换到MiniPC Lock模式时
				gimbal_mode_change->gimbal_yaw_motor.absolute_angle_set = gimbal_mode_change->gimbal_yaw_motor.absolute_angle;
				gimbal_mode_change->gimbal_yaw_motor.miniPC_absolute_angle_set = gimbal_mode_change->gimbal_yaw_motor.absolute_angle;
		}
    ...
    else if (gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_ENCONDE && gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
		...
		else if (gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK && gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK)
		{//切换到MiniPC Lock模式时
				gimbal_mode_change->gimbal_pitch_motor.absolute_angle_set = gimbal_mode_change->gimbal_pitch_motor.absolute_angle;
				gimbal_mode_change->gimbal_pitch_motor.miniPC_absolute_angle_set = gimbal_mode_change->gimbal_pitch_motor.absolute_angle;
		}
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
    ...
    else if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
		...
		{
				//miniPC 绝对角度 锁定gimbal_miniPC_autoaim_lock_absolute_angle_control
				gimbal_miniPC_autoaim_lock_absolute_angle_limit(&set_control->gimbal_pitch_motor, 0);
		}
}
/**
  * @brief          gimbal control mode :GIMBAL_MOTOR_GYRO, use euler angle calculated by gyro sensor to control. 
...
static void gimbal_miniPC_autoaim_lock_absolute_angle_limit(gimbal_motor_t *gimbal_motor, uint8_t bypass)
{
    ...
    if (gimbal_motor == NULL)
    {
		...
		if(bypass == 0)
		{
			//gimbal_motor->miniPC_absolute_angle_set //是刚刚从miniPC读取到的新的目标角度
			//gimbal_motor->absolute_angle_set等价于 last miniPC_absolute_angle_set
			//先算出当前误差角度 即上一次执行之后的误差角度
			bias_angle = rad_format(gimbal_motor->absolute_angle_set - gimbal_motor->absolute_angle);
			
			//新增角度
			add_angle = gimbal_motor->miniPC_absolute_angle_set - gimbal_motor->absolute_angle_set;
			
			//relative angle + angle error + add_angle > max_relative angle
			//云台相对角度+ 误差角度 + 新增角度 如果大于 最大机械角度
			if (gimbal_motor->relative_angle + bias_angle + add_angle > gimbal_motor->max_relative_angle)
			{
				//如果是往最大机械角度控制方向
				if(add_angle > 0.0f)
				{
					add_angle = gimbal_motor->max_relative_angle - gimbal_motor->relative_angle - bias_angle;
				}
			}
			else if(gimbal_motor->relative_angle + bias_angle + add_angle < gimbal_motor->min_relative_angle)
			{
				if(add_angle < 0.0f)
				{
					add_angle = gimbal_motor->min_relative_angle - gimbal_motor->relative_angle - bias_angle;
				}
			}
			//加上去经过限幅的角度
			angle_set = gimbal_motor->absolute_angle_set;
			gimbal_motor->absolute_angle_set = rad_format(angle_set + add_angle);
		}
		else
		{//直接赋值, 不进行限幅
			gimbal_motor->absolute_angle_set = rad_format(gimbal_motor->miniPC_absolute_angle_set);
		}
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
		...
		else if(control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK)
		{
				gimbal_motor_miniPC_autoaim_lock_absolute_angle_control(&control_loop->gimbal_yaw_motor);
		}
    ...
    else if (control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
		...
		else if(control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_MINIPC_AUTOAIM_LOCK)
		{
				gimbal_motor_miniPC_autoaim_lock_absolute_angle_control(&control_loop->gimbal_pitch_motor);
		}
}
...
static void gimbal_motor_miniPC_autoaim_lock_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
		if (gimbal_motor == NULL)
    {
		...
		//SZL 6-12-2022 修改 用于 MiniPC 角度环 绝对角度 的控制 速度环串级pid调试
		gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->miniPC_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, gimbal_motor->motor_gyro);
...
//		{
//			//SZL 6-12-2022 修改 用于 MiniPC 角度环 绝对角度 的控制
//			gimbal_motor->motor_gyro_set = gimbal_PID_calc(&gimbal_motor->miniPC_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->miniPC_absolute_angle_set, gimbal_motor->motor_gyro);
//		}
//		else
//		{
		//角度环，速度环串级pid调试
